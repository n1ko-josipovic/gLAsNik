    ...
    class PacketReader : BinaryReader
    {
        ...
        public async Task<string> ReadMessageAsync()
        {
            ...
            while (read < 4)
            {
                ...
                if (r == 0) throw new IOException("Disconnected while reading length.");
                read += r;
            }
            ...
            while (read < length)
            {
                ...
                if (r == 0) throw new IOException("Disconnected while reading message.");
                read += r;
            }
        ...
        private async Task ReadExactAsync(byte[] buffer, int count)
        {
            ...
            while (offset < count)
            {
                ...
                if (bytesRead == 0)
                    throw new IOException("Disconnected while reading data.");
                offset += bytesRead;
            }
        }
    }
